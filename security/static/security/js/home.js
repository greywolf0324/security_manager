...
$(document).ready(function() {
              ...
              ${
                keys.map(key => {
                  if (key === "Vendor Style from OMS_equal") {
                    ...
                    return `<td contenteditable="true"><select class="form-select" ${hasValue?"disabled":""} data-hasvalue="${hasValue}"><option value="" disabled selected>Select UOMStockLocation</option>${options["uom_options"][i][index - 1].map(option => `<option value="${option}" ${option==selectedValue?"selected":""}>${option}</option>`)}</select></td>`;
                  }
  ...
  // $('#history_viewer').click(function() {
  //   console.log("<>>>>>>>>>>>>>>>>>")
  //   $.ajax({
  //     url: '/history_view/<str:creation_date>/',
  //     type: 'GET',
  //     success: function(response) {
  //       console.log("response")
  //       // const { customername, headerkeys, itemkeys, headerDetails, itemDetails } = response;
  //       // // const [_customername, headerDetails, itemDetails] = JSON.parse(response.res)
  //       // $(".review-stepper .customername").html(_customername);
  //       // $(".modal-backdrop.fade.show").remove()
  //       // historyBuildTable(headerkeys, itemkeys, headerDetails, itemDetails, "#history_view_details", single=false)
  //     },
  //     error: function(xhr, status, error) {
  //       console.log("errr")
  //     }
  ...
  // function historyBuildTable(keys_header, keys_item, data_header, data_item, selector, single=true) {
  ...
  //   console.log("data_item", data_item)
    ...
    // let tmpHtml = '';
    //   // for (let x = 0; x < data_header.length; x++) {
    //       for (let j = 0; j < keys_header[0].length; j++) {
    //         tmpHtml += `<div class="w-100 d-flex p-4 customer-item" onClick="customerPODetails('${selector}', ${x})">
    //           <div class="w-50px d-flex justify-center pt-3">
    //             <i class="fas fs-20px fa-address-card"></i>
    //           </div>
    ...
    //             <p class="mb-0">
    //               <strong>Customer PO:</strong>
    //               <span class="text-info">${keys_header[0][j]}</span>
    //             </p>
    //             <p class="mb-0">selector
    //               PO Date:
    //               <span class="text-info">${keys_header[1][j]}</span>
    //             </p>
    //             <p class="mb-0">
    //               Ship Date:
    //               <span class="text-info">${keys_header[3][j]}</span>
    //             </p>
    //             <p class="mb-0">
    //               PO Total:
    //               <span class="text-info">${Number(keys_header[6][j]).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</span>
    //             </p>
    //             </div>
    //           </div>
    //         </div>`
